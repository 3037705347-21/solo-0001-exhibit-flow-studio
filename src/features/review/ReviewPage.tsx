import { AlertCircle, Check, CheckCircle2, ClipboardCheck, Clock, Download, FileWarning, GitBranch, ListChecks, MapPin, Package, Plus, RotateCcw, Send, ShieldAlert, Sparkles, UserRound, XCircle } from 'lucide-react';
...
import { checkExportDependencies } from '../../domain/reviewRules';
import { downloadTextFile } from '../../domain/export';
import { sortZones } from '../../domain/filters';
import { formatDate } from '../../domain/formatters';
import { analyzeJourney } from '../../domain/journeyAnalysis';
import { checkSnapshotDependencies, issueNodeId } from '../../domain/lineage';
import { issueLineage } from '../../domain/lineageView';
...
import { selectPublishedSnapshots } from '../../state/selectors';
...
export function ReviewPage() {
  const { state, addIssue, transitionReviewIssue, checkReadiness, createSnapshot, acknowledgeLineage } = useWorkspace();
  ...
  const [lineageIssue, setLineageIssue] = useState<ReviewIssue | null>(null);
  ...
  const published = selectPublishedSnapshots(state);
  ...
  const exportSnapshot = () => {
    ...
    const stale = result.value.lineage.needsReviewCount;
    notify(stale > 0
      ? `Snapshot downloaded with dependency closure · ${stale} record${stale === 1 ? '' : 's'} flagged for re-review.`
      : 'Snapshot downloaded with full dependency closure.');
  };
  ...
  const lineageView = lineageIssue ? issueLineage(state, lineageIssue.id) : null;
  ...
  return <div className="page-stack"><SectionHeader eyebrow="QUALITY GATE" title="Review desk" description="Turn open questions into resolved decisions, then run the final readiness check." actions={<div className="header-button-row"><Button variant="secondary" icon={<ClipboardCheck size={16} />} onClick={runCheck}>Run readiness check</Button><Button variant="primary" icon={<Plus size={17} />} onClick={() => setShowModal(true)}>New finding</Button></div>} />
    ...
    {!readiness.ready && <section className="blocker-list">{readiness.blockers.map((blocker) => <div className="blocker-row" key={blocker}><XCircle size={16} /><span>{blocker}</span></div>)}</section>}
    <DependencyCheckCard exportCheck={exportCheck} published={published} state={state} onAcknowledge={(nodeId) => {
      const artifactId = nodeId.startsWith('artifact:') ? nodeId.slice('artifact:'.length) : undefined;
      acknowledgeLineage(nodeId, artifactId);
    }} />
    ...
    <div className="review-toolbar"><div className="segmented-control">{STATUS_FILTERS.map((status) => <button key={status} className={filter === status ? 'selected' : ''} onClick={() => setStatus(status)}>{status === 'all' ? 'All' : status[0].toUpperCase() + status.slice(1).replace('-', ' ')} <span>{counts[status]}</span></button>)}</div></div>
    {checklist && <ZoneChecklistCard checklist={checklist} onDownload={exportChecklist} />}
    <section className="issue-list">{filtered.map((issue) => <IssueRow key={issue.id} issue={issue} state={state} onTransition={(status) => transitionReviewIssue(issue.id, status)} onShowLineage={() => setLineageIssue(issue)} />)}</section>
    ...
    {lineageIssue && lineageView && <Modal eyebrow="FINDING PROVENANCE" title={lineageIssue.title} onClose={() => setLineageIssue(null)} footer={<Button variant="ghost" onClick={() => setLineageIssue(null)}>Close</Button>}><LineagePanel view={lineageView} onAcknowledge={() => { acknowledgeLineage(issueNodeId(lineageIssue.id)); setLineageIssue(null); notify('Finding marked as reviewed against current sources.'); }} /></Modal>}
    {toast && <div className="toast toast-positive"><Download size={16} />{toast}</div>}
  </div>;
}

function DependencyCheckCard({ exportCheck, published, state, onAcknowledge }: { exportCheck: ReturnType<typeof checkExportDependencies>; published: ReturnType<typeof selectPublishedSnapshots>; state: ReturnType<typeof useWorkspace>['state']; onAcknowledge: (nodeId: string) => void }) {
  const [openPackage, setOpenPackage] = useState<string | null>(null);
  const packageCheck = openPackage ? checkSnapshotDependencies(state, openPackage) : null;
  return <section className="dependency-card">
    <div className="panel-heading"><div><div className="eyebrow"><Package size={12} /> PUBLISHED PACKAGE DEPENDENCIES</div><h2>Export lineage</h2></div>{exportCheck.deletedCount > 0
      ? <Badge tone="danger">{exportCheck.deletedCount} deleted source{exportCheck.deletedCount === 1 ? '' : 's'}</Badge>
      : exportCheck.needsReviewCount > 0
        ? <Badge tone="warning">{exportCheck.needsReviewCount} need re-review</Badge>
        : <Badge tone="positive">Closure current</Badge>}</div>
    {exportCheck.blockers.length > 0 && <div className="dependency-blockers">{exportCheck.blockers.map((blocker) => <div className="blocker-row" key={blocker}><XCircle size={15} /><span>{blocker}</span></div>)}</div>}
    {exportCheck.blockers.length === 0 && exportCheck.cautions.length > 0 && <div className="dependency-cautions">{exportCheck.cautions.map((caution) => <div className="caution-row" key={caution}><AlertCircle size={15} /><span>{caution} Export is allowed but the package records the re-review state.</span></div>)}</div>}
    {exportCheck.blockers.length === 0 && exportCheck.cautions.length === 0 && <p className="dependency-ok"><CheckCircle2 size={15} /> Every source feeding a prospective export matches its last-reviewed content.</p>}
    {published.length > 0 && <div className="package-list">{published.slice(0, 4).map((node) => {
      const check = checkSnapshotDependencies(state, node.id);
      return <button className={`package-row ${check.needsReview.length ? 'is-stale' : ''}`} key={node.id} onClick={() => setOpenPackage(node.id)}>
        <Package size={14} />
        <span className="package-row-main"><strong>{node.label}</strong><small>{formatDate(node.createdAt)} · {check.dependencies.length} dependencies</small></span>
        {check.deletedSources.length > 0
          ? <Badge tone="danger">{check.deletedSources.length} deleted</Badge>
          : check.needsReview.length > 0
            ? <Badge tone="warning">{check.needsReview.length} re-review</Badge>
            : <Badge tone="positive">Current</Badge>}
      </button>;
    })}</div>}
    {packageCheck && openPackage && <Modal eyebrow="PUBLISHED PACKAGE" title={packageCheck.dependencies.length ? `Dependencies (${packageCheck.dependencies.length})` : 'Package dependencies'} onClose={() => setOpenPackage(null)} footer={<Button variant="ghost" onClick={() => setOpenPackage(null)}>Close</Button>}>
      <div className="dependency-detail">
        {packageCheck.blockers.map((blocker) => <div className="blocker-row" key={blocker}><XCircle size={15} /><span>{blocker}</span></div>)}
        {packageCheck.cautions.map((caution) => <div className="caution-row" key={caution}><AlertCircle size={15} /><span>{caution}</span></div>)}
        {packageCheck.dependencies.map((dependency) => <div className={`dependency-row ${dependency.status === 'needs-review' ? 'is-stale' : ''}`} key={dependency.nodeId}>
          <span className={`lineage-dot lineage-dot-${dependency.status === 'needs-review' ? (dependency.staleReason === 'source-deleted' || dependency.staleReason === 'source-removed' ? 'deleted' : 'needs-review') : 'valid'}`} />
          <span className="dependency-row-main"><strong>{dependency.label}</strong><small>{dependency.type}{dependency.origin === 'import' ? ` · imported${dependency.batchFileName ? ` from ${dependency.batchFileName}` : ''}` : ''}{dependency.dependsOn.length ? ` · ${dependency.dependsOn.length} source${dependency.dependsOn.length === 1 ? '' : 's'}` : ''}</small></span>
          {dependency.status === 'needs-review'
            ? <Button variant="ghost" onClick={() => onAcknowledge(dependency.nodeId)}>Mark reviewed</Button>
            : <Badge tone="positive">Current</Badge>}
        </div>)}
      </div>
    </Modal>}
...
function IssueRow({ issue, state, onTransition, onShowLineage }: { issue: ReviewIssue; state: ReturnType<typeof useWorkspace>['state']; onTransition: (status: ReviewIssue['status']) => { ok: boolean; message?: string }; onShowLineage: () => void }) {
  ...
  const view = issueLineage(state, issue.id);
  const sourceDeleted = view.staleReason === 'source-deleted';
  const staleLabel = sourceDeleted
    ? (issue.artifactId ? 'Linked object deleted' : 'Source removed')
    : 'Source changed';
  ...
  return <article className={`issue-row issue-${issue.severity} ${view.staleReason ? 'is-lineage-stale' : ''}`}><div className="issue-severity">{issue.severity === 'critical' ? <ShieldAlert size={19} /> : issue.severity === 'warning' ? <AlertCircle size={19} /> : <FileWarning size={19} />}</div><div className="issue-main"><div className="issue-title-line"><h3>{issue.title}</h3><Badge tone={issue.status === 'resolved' ? 'positive' : issue.severity === 'critical' ? 'danger' : issue.severity === 'warning' ? 'warning' : 'neutral'}>{issue.status === 'in-progress' ? 'In Progress' : issue.status[0].toUpperCase() + issue.status.slice(1)}</Badge>{view.staleReason && <Badge tone={sourceDeleted ? 'danger' : 'warning'}><GitBranch size={10} /> Needs re-review</Badge>}</div><p>{issue.description}</p><div className="issue-meta"><span><UserRound size={13} /> {issue.owner}</span>{issue.zoneId && <span><MapPin size={13} /> Zone linked</span>}{issue.artifactId && <span>Object linked</span>}<span>Updated {formatDate(issue.updatedAt)}</span></div>{view.staleReason && <button className="lineage-inline-link" onClick={onShowLineage}><GitBranch size={12} /> {staleLabel} — open provenance to review</button>}{error && <div className="field-error">{error}</div>}</div><Button variant="ghost" icon={<GitBranch size={15} />} aria-label={`Show provenance for ${issue.title}`} onClick={onShowLineage} /><Button variant={issue.status === 'resolved' ? 'ghost' : 'secondary'} icon={issue.status === 'resolved' ? <RotateCcw size={15} /> : <Check size={15} />} onClick={result}>{resultLabel}</Button></article>;
}
