...
import {
  analyzeJourney,
  canPlaceArtifact,
  getUnplacedArtifacts,
} from '../../domain/journeyAnalysis';
...
export function JourneyPage() {
  ...
  return (
    <div className="page-stack">
      <SectionHeader
        eyebrow="SPATIAL EDITOR"
        title="Visitor journey"
        description="Arrange the sequence, then let the constraint engine challenge the story."
        actions={
          <div className="inline-status">
            <Route size={16} />
            <span>{formatMinutes(analysis.totalDwellMinutes)} planned visit</span>
          </div>
        }
      />
      <div className="metric-grid four">
        <Metric
          ...
          value={`${analysis.placedCount}/${state.artifacts.length}`}
        ...
        <Metric
          label="Key coverage"
          value={formatPercent(analysis.keyObjectCoverage)}
          detail="Required objects"
          icon={<ShieldCheck size={17} />}
          tone={analysis.keyObjectCoverage === 1 ? 'teal' : 'red'}
        />
        <Metric
          label="Role coverage"
          ...
          {unplaced.length ? (
            <div className="unplaced-list">
              {unplaced.map((artifact) => (
                <button
                  className={`unplaced-item ${selectedArtifact === artifact.id ? 'selected' : ''}`}
                  key={artifact.id}
                  onClick={() =>
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      {artifact.accessionId} · {titleCase(artifact.narrativeRole)}
                    </small>
                  </span>
                  <Plus size={15} />
                </button>
            ...
            <EmptyState
              ...
              detail="Every collection object has a place in the visitor journey."
            />
          )}
...
function ZoneLane({
  ...
  onReorder: (zoneId: string, artifactId: string, direction: -1 | 1) => void;
}) {
  void onSelect;
  ...
  return (
    <article className="zone-lane">
      <div className="zone-marker" style={{ backgroundColor: zone.color }}>
      ...
      <div className="zone-content">
        <div className="zone-head">
          ...
          <div className="zone-stats">
            <span>
              {placed.length}/{zone.maxObjects} objects
            </span>
            <span>
              {analysis?.dwellMinutes ?? 0}/{zone.capacityMinutes} min
            </span>
          </div>
        </div>
        <ProgressBar
          value={(analysis?.utilization ?? 0) * 100}
          tone={issueCount ? 'red' : (analysis?.utilization ?? 0) >= 0.8 ? 'amber' : 'teal'}
        />
        ...
        <div className="placement-list">
          {placed.map((artifact, artifactIndex) => (
            <div className="placement-item" key={artifact.id}>
              <GripVertical size={15} className="drag-handle" />
              <ArtifactGlyph color={artifact.color} size="small" />
              <div className="placement-info">
                <strong>{artifact.title}</strong>
                <span>
                  {titleCase(artifact.narrativeRole)} · {artifact.dwellMinutes} min
                </span>
              </div>
              <div className="placement-actions">
                <Button
                  ...
                  onClick={() => onReorder(zone.id, artifact.id, -1)}
                />
                <Button
                  variant="ghost"
                  icon={<ArrowDown size={14} />}
                  aria-label={`Move ${artifact.title} down`}
                  disabled={artifactIndex === placed.length - 1}
                  onClick={() => onReorder(zone.id, artifact.id, 1)}
                />
                <Button
                  variant="ghost"
          ...
          {placed.length === 0 && !selected && (
            <div className="zone-empty">Select an object from the queue to place it here.</div>
          )}
        </div>
      </div>
    </article>
