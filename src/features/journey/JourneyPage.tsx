...
export function JourneyPage() {
  ...
  useEffect(() => {
    ...
    if (artifactId && unplaced.some((artifact) => artifact.id === artifactId)) setSelectedArtifact(artifactId);
    const zoneId = searchParams.get('zone');
    if (zoneId) document.getElementById(`zone-lane-${zoneId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    // Run once per navigation; query params are deep links from the command center.
