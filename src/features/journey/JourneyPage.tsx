import { AlertTriangle, ArrowDown, ArrowUp, CheckCircle2, Eye, GripVertical, Lightbulb, Minus, Plus, Route, ShieldAlert, ShieldCheck, Sparkles, XCircle } from 'lucide-react';
...
export function JourneyPage() {
  const { state, assignArtifact, removePlacement, reorderArtifact, boundProfile, ruleResolution } = useWorkspace();
  ...
  const place = (artifact: Artifact, zone: Zone) => {
    if (!boundProfile || !analysis) { setNotice(ruleResolution.reason); return; }
    const preview = canPlaceArtifact(artifact, zone, boundProfile.parameters);
  ...
  return <div className="page-stack"><SectionHeader eyebrow="SPATIAL EDITOR" title="Visitor journey" description="Arrange the sequence, then let the constraint engine challenge the story." actions={<div className="inline-status">{analysis ? <><Route size={16} /><span>{formatMinutes(analysis.totalDwellMinutes)} planned visit</span></> : <><ShieldAlert size={16} /><span>Awaiting a bound rule archive</span></>}</div>} />
    {boundProfile && <div className="rule-archive-chip"><Sparkles size={14} /><span>Evaluated with <strong>{profileLabel(boundProfile)}</strong></span></div>}
    <RuleArchiveBanner />
