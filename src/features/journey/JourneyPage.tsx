import { AlertTriangle, ArrowDown, ArrowUp, CheckCircle2, Eye, GripVertical, Lightbulb, Minus, Plus, Route, ShieldCheck, Sparkles, XCircle } from 'lucide-react';
...
export function JourneyPage() {
  const { state, assignArtifact, removePlacement, reorderArtifact, beginInteraction, endInteraction } = useWorkspace();
  ...
  // Pin the base revision while an object is armed for placement so a placement racing another tab is detected.
  const selectArtifact = (id: string | null) => {
    if (id) beginInteraction();
    else endInteraction();
  ...
  const place = (artifact: Artifact, zone: Zone) => {
    ...
    if (!result.ok) {
      // A version conflict opens the review dialog; keep the object armed so the
      // choice can be replayed without rebuilding the placement.
      if (!result.message?.includes('older workspace version')) {
  ...
  const warn = (message: string) => { setNotice(message); window.setTimeout(() => setNotice(null), 3200); };
  return <div className="page-stack"><SectionHeader eyebrow="SPATIAL EDITOR" title="Visitor journey" description="Arrange the sequence, then let the constraint engine challenge the story." actions={<div className="inline-status"><Route size={16} /><span>{formatMinutes(analysis.totalDwellMinutes)} planned visit</span></div>} />
    ...
    <div className="journey-layout"><section className="journey-board"><div className="board-header"><div><div className="eyebrow">SEQUENCE BOARD</div><h2>Visitor flow</h2></div><div className="board-legend"><span><i className="legend-dot legend-placed" /> placed</span><span><i className="legend-dot legend-issue" /> needs attention</span></div></div><div className="zone-list">{state.zones.slice().sort((a, b) => a.sequence - b.sequence).map((zone, index) => <ZoneLane key={zone.id} zone={zone} index={index} artifacts={state.artifacts} analysis={analysis.zones.find((item) => item.zoneId === zone.id)} selectedArtifact={selectedArtifact} onSelect={selectArtifact} onPlace={place} onRemove={(artifactId) => { const result = removePlacement(artifactId); if (result && !result.ok) warn(result.message ?? 'Placement could not be removed.'); }} onReorder={(zoneId, artifactId, direction) => { const result = reorderArtifact(zoneId, artifactId, direction); if (!result.ok) warn(result.message ?? 'Object sequence could not be changed.'); }} />)}</div></section><aside className="journey-sidebar"><div className="panel-heading"><div><div className="eyebrow">UNPLACED</div><h3>Object queue</h3></div><Badge tone={unplaced.length ? 'warning' : 'positive'}>{unplaced.length}</Badge></div>{unplaced.length ? <div className="unplaced-list">{unplaced.map((artifact) => <button className={`unplaced-item ${selectedArtifact === artifact.id ? 'selected' : ''}`} key={artifact.id} onClick={() => selectArtifact(selectedArtifact === artifact.id ? null : artifact.id)}><ArtifactGlyph color={artifact.color} size="small" /><span><strong>{artifact.title}</strong><small>{artifact.accessionId} · {titleCase(artifact.narrativeRole)}</small></span><Plus size={15} /></button>)}</div> : <EmptyState icon={<CheckCircle2 size={22} />} title="Queue is clear" detail="Every collection object has a place in the visitor journey." />}<div className="constraint-panel"><div className="panel-heading"><div><div className="eyebrow">ANALYSIS</div><h3>Constraint review</h3></div><Badge tone={analysis.blockingCount ? 'danger' : 'positive'}>{analysis.blockingCount ? 'Blocked' : 'Clear'}</Badge></div>{analysis.findings.length ? <div className="finding-list compact">{analysis.findings.slice(0, 5).map((finding) => <div className={`finding-row ${finding.type}`} key={finding.id}>{finding.type === 'error' ? <XCircle size={15} /> : finding.type === 'warning' ? <AlertTriangle size={15} /> : <Lightbulb size={15} />}<span><strong>{finding.title}</strong><small>{finding.detail}</small></span></div>)}</div> : <div className="clear-message"><CheckCircle2 size={17} /> No constraints detected</div>}</div></aside></div>{notice && <div className="toast toast-warning"><AlertTriangle size={16} />{notice}</div>}</div>;
}
