...
import { BlockerContextBanner } from '../../components/BlockerContextBanner';
...
export function JourneyPage() {
  ...
  const focusZone = focusZoneId ? state.zones.find((zone) => zone.id === focusZoneId) : undefined;
  useEffect(() => {
    if (!focusZone) return;
    document.getElementById(`journey-zone-${focusZone.id}`)?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
  }, [focusZone]);
  ...
  return <div className="page-stack"><SectionHeader eyebrow="SPATIAL EDITOR" title="Visitor journey" description="Arrange the sequence, then let the constraint engine challenge the story." actions={<div className="inline-status"><Route size={16} /><span>{formatMinutes(analysis.totalDwellMinutes)} planned visit</span></div>} />
    <BlockerContextBanner />
    {focusZoneId && !focusZone && <InlineNotice tone="warning" message="The linked zone is no longer part of the journey. Return to the review desk and re-run the readiness check." />}
