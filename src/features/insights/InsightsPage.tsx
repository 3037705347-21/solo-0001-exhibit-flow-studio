...
export function InsightsPage() {
  ...
  const apply = () => {
    ...
    } else if (!result.message?.includes('older workspace version')) {
      setApplyError(result.message ?? 'Preferences could not be saved.');
      window.setTimeout(() => setApplyError(null), 3200);
    }
  };
  return <div className="page-stack"><SectionHeader eyebrow="SCENARIO LAB" title="Insights" description="Pressure-test the plan for different visitor rhythms without changing the saved journey." actions={<Button variant="primary" icon={<Save size={16} />} onClick={apply}>Apply preferences</Button>} />
    ...
    <section className="pressure-section"><div className="panel-heading"><div><div className="eyebrow">ZONE PRESSURE</div><h2>Where the plan is carrying weight</h2></div><Layers3 size={19} /></div><div className="pressure-grid">{analysis.zones.map((zone) => { const pressured = projection.pressureZoneIds.includes(zone.zoneId); const source = state.zones.find((candidate) => candidate.id === zone.zoneId); return <div className={`pressure-card ${pressured ? 'pressured' : ''}`} key={zone.zoneId}><div className="pressure-card-top"><span className="zone-color" style={{ backgroundColor: source?.color }} /><strong>{source?.shortLabel}</strong>{pressured && <Badge tone="warning">Pressure</Badge>}</div><div className="pressure-stat"><span>{zone.objectCount} objects</span><strong>{zone.dwellMinutes} min</strong></div><ProgressBar value={zone.utilization * 100} tone={pressured ? 'amber' : 'teal'} /></div>; })}</div></section>{saved && <div className="toast toast-positive"><Save size={16} />Planning preferences applied.</div>}{applyError && <div className="toast toast-warning"><Info size={16} />{applyError}</div>}</div>;
}
