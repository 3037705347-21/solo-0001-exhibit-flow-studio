...
export function InsightsPage() {
  ...
  const analysis = useMemo(
    () => analyzeJourney(state.artifacts, state.zones),
  ...
  const setPreference = <K extends keyof PlanningPreferences>(
    key: K,
  ...
  return (
    <div className="page-stack">
      <SectionHeader
        ...
        title="Insights"
        description="Pressure-test the plan for different visitor rhythms without changing the saved journey."
        actions={
      ...
      <div className="scenario-banner">
        ...
        <div>
          ...
          <p>
            Try a visitor profile to see how pacing, group size, and access priorities reshape the
            visit.
          </p>
        </div>
        ...
        <section className="scenario-controls">
          ...
          <div className="control-block">
            ...
            <div className="pace-options">
              {(['focused', 'balanced', 'leisurely'] as const).map((pace) => (
                <button
                  key={pace}
          ...
          <div className="control-block">
            <div className="control-label-row">
              ...
              <strong>{draft.groupSize} people</strong>
            </div>
            <input
              id="group-size"
              ...
              onChange={(event) => setPreference('groupSize', Number(event.target.value))}
            />
          ...
          <div className="control-block">
            <div className="control-label-row">
              ...
              <strong>{draft.accessibilityPriority}%</strong>
            </div>
            <input
              id="access-priority"
              ...
              min="0"
              max="100"
              value={draft.accessibilityPriority}
              onChange={(event) =>
                setPreference('accessibilityPriority', Number(event.target.value))
              }
            />
            <div className="range-labels">
              <span>Baseline</span>
          ...
          <div className="saved-profile">
            <Info size={15} />
            <span>
              Saved profile: <strong>{titleCase(state.preferences.pace)}</strong> pace ·{' '}
              {state.preferences.groupSize} people
            </span>
          </div>
        </section>
        <section className="projection-panel">
          <div className="panel-heading">
            <div>
              ...
              <h2>{titleCase(draft.pace)} visit</h2>
            </div>
            <Badge tone={projection.comfortScore >= 75 ? 'positive' : 'warning'}>
              {projection.comfortScore >= 75 ? 'Comfortable' : 'Pressure points'}
            </Badge>
          </div>
          <div className="projection-metrics">
            <Metric
              label="Expected duration"
              value={formatMinutes(projection.durationMinutes)}
              detail="At this visitor pace"
              icon={<Gauge size={17} />}
              tone="teal"
            />
            <Metric
            ...
            <Metric
              ...
              value={`${projection.accessibilityScore}/100`}
              detail="Interpretation access"
              icon={<Accessibility size={17} />}
              tone={projection.accessibilityScore >= 75 ? 'teal' : 'amber'}
            />
          </div>
          <div className="donut-row">
            <Donut value={projection.narrativeScore / 100} color="#c9563f" label="Narrative" />
            <Donut value={projection.accessibilityScore / 100} color="#2f7c75" label="Access" />
            <Donut value={projection.comfortScore / 100} color="#7c6aa6" label="Comfort" />
          </div>
      ...
      <section className="pressure-section">
        <div className="panel-heading">
          <div>
            <div className="eyebrow">ZONE PRESSURE</div>
            <h2>Where the plan is carrying weight</h2>
          </div>
          <Layers3 size={19} />
        </div>
        <div className="pressure-grid">
          {analysis.zones.map((zone) => {
            const pressured = projection.pressureZoneIds.includes(zone.zoneId);
            const source = state.zones.find((candidate) => candidate.id === zone.zoneId);
            return (
              <div className={`pressure-card ${pressured ? 'pressured' : ''}`} key={zone.zoneId}>
                <div className="pressure-card-top">
                  <span className="zone-color" style={{ backgroundColor: source?.color }} />
                  <strong>{source?.shortLabel}</strong>
                  {pressured && <Badge tone="warning">Pressure</Badge>}
                </div>
                <div className="pressure-stat">
                  <span>{zone.objectCount} objects</span>
                  <strong>{zone.dwellMinutes} min</strong>
                </div>
                <ProgressBar value={zone.utilization * 100} tone={pressured ? 'amber' : 'teal'} />
              </div>
            );
          })}
        </div>
