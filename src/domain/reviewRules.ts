...
export function evaluateReadiness(
  ...
  if (analysis.blockingCount > 0) {
    blockers.push(
      `${analysis.blockingCount} blocking journey constraint${analysis.blockingCount === 1 ? '' : 's'} remain.`,
    );
  }
  if (unresolvedCritical.length > 0) {
    blockers.push(
      `${unresolvedCritical.length} critical review finding${unresolvedCritical.length === 1 ? '' : 's'} remain unresolved.`,
    );
  }
  if (analysis.placedCount === 0) blockers.push('The visitor journey has no placed objects.');
  if (analysis.keyObjectCoverage < 1)
    blockers.push('Every key object must be placed in the journey.');
  if (analysis.roleCoverage < 1)
    blockers.push('The planned journey does not cover every narrative role.');

  if (analysis.warningCount > 0)
    cautions.push(
      `${analysis.warningCount} journey warning${analysis.warningCount === 1 ? '' : 's'} should be reviewed.`,
    );
  if (unresolvedWarnings.length > 0)
    cautions.push(
      `${unresolvedWarnings.length} non-critical review finding${unresolvedWarnings.length === 1 ? '' : 's'} remain open.`,
    );
  if (analysis.unplacedCount > 0)
    cautions.push(
      `${analysis.unplacedCount} collection object${analysis.unplacedCount === 1 ? '' : 's'} are not used.`,
    );
