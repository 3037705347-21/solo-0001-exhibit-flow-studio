...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const upsertArtifact = useCallback(
    (draft: ArtifactDraft, existing?: Artifact): CommandResult<Artifact> => {
  ...
  const removeArtifact = useCallback(
    (artifactId: string): CommandResult => {
  ...
  const assignArtifact = useCallback((artifactId: string, zoneId: string): CommandResult => {
    try {
      ...
      return {
        ok: false,
        message: error instanceof Error ? error.message : 'Placement could not be updated.',
      };
    }
  }, []);
  ...
  const reorderArtifact = useCallback(
    (zoneId: string, artifactId: string, direction: -1 | 1): CommandResult => {
      try {
        ...
        return {
          ok: false,
          message: error instanceof Error ? error.message : 'Object sequence could not be changed.',
        };
      }
    },
    [],
  ...
  const transitionReviewIssue = useCallback(
    (issueId: string, status: IssueStatus): CommandResult => {
      ...
      try {
        ...
        return {
          ok: false,
          message: error instanceof Error ? error.message : 'Status could not be changed.',
        };
      }
    },
    [state.issues],
