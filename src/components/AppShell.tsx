...
import { evaluateApproval } from '../domain/approval';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const stageBadge = state.project.stage !== 'ready'
    ? { tone: 'warning' as const, label: 'In review' }
    : evaluateApproval(state).status === 'active'
      ? { tone: 'positive' as const, label: 'Signed off' }
      : { tone: 'warning' as const, label: 'Awaiting sign-off' };
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb"><LayoutDashboard size={15} /><span>ExhibitFlow</span><ChevronRight size={14} /><strong>{active.label}</strong></div><div className="topbar-actions"><Badge tone={stageBadge.tone}>{stageBadge.label}</Badge><Button variant="ghost" icon={<Settings2 size={17} />} aria-label="Open settings" /></div></header><div className="page-content">{children}</div><footer className="app-footer"><span>ExhibitFlow Studio · offline workspace</span><span>Changes save automatically</span></footer></main>
  </div>;
}
