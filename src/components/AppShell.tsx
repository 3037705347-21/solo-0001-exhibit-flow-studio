...
const navigation = [
  { to: '/command', label: 'Command center', icon: LayoutDashboard, detail: 'Project status' },
...
const stageBadge = {
  ...
  ready: { tone: 'positive' as const, label: 'Ready to share' },
};
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb"><LayoutDashboard size={15} /><span>ExhibitFlow</span><ChevronRight size={14} /><strong>{active.label}</strong></div><div className="topbar-actions"><Badge tone={stageBadge[state.project.stage].tone}>{stageBadge[state.project.stage].label}</Badge><Button variant="ghost" icon={<Settings2 size={17} />} aria-label="Open settings" /></div></header><div className="page-content">{children}</div><footer className="app-footer"><span>ExhibitFlow Studio · offline workspace</span><span>Changes save automatically</span></footer></main>
  </div>;
}
