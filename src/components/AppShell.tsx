...
import { getDeletionStatus } from '../domain/deletion';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  const { state, storageHealthy, resetWorkspace, recoveryOpen, openRecovery, closeRecovery } = useWorkspace();
  const location = useLocation();
  const active = navigation.find((item) => location.pathname.startsWith(item.to)) ?? navigation[0];
  const recoverableCount = state.deletionRecords.filter((record) => getDeletionStatus(record) === 'recoverable').length;
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        <Button variant="ghost" icon={<History size={15} />} className="recovery-entry" onClick={openRecovery}>
          <span className="recovery-entry-label">Recovery center</span>
          {recoverableCount > 0 && <Badge tone="warning">{recoverableCount}</Badge>}
        </Button>
        <div className="sidebar-note"><CircleHelp size={16} /><span><strong>Planning tip</strong><small>Keep each zone's thesis visible while you place objects.</small></span></div><div className="storage-status"><span className={`status-dot ${storageHealthy ? 'online' : 'offline'}`} />{storageHealthy ? 'Saved locally' : 'Local save unavailable'}</div><Button variant="ghost" icon={<RotateCcw size={15} />} onClick={() => { if (window.confirm('Reset this workspace to the sample exhibition? Deletion history and the published-package ledger reset with it.')) resetWorkspace(); }}>Reset sample plan</Button></div>
    </aside>
    ...
    <RecoveryCenter open={recoveryOpen} onClose={closeRecovery} />
  </div>;
}
